...
public class RelationValidatorTest {
    ...
    public void testRelationFail() throws Exception {
        ...
        Assert.assertEquals(
            "L'objet test_3 de la table TEST doit faire référence à un objet de la table N_prefixTri_INONDABLE_suffixInond_S_ddd via l'attribut ID_S_INOND. L'attribut n'est pas renseigné (s_inond_1) ou alors la relation n'est pas vérifiée.",
            refError1.getMessage()
        );

        Assert.assertEquals(
            "L'objet test_b1 de la table TEST_B doit faire référence à un objet de la table N_prefixTri_INONDABLE_suffixInond_S_ddd via l'attribut ID_S_INOND. L'attribut n'est pas renseigné (s_inond_1) ou alors la relation n'est pas vérifiée.",
            refError2.getMessage()
        );

        Assert.assertEquals(
            "L'objet test_b1 de la table TEST_B doit faire référence à un objet de la table N_prefixTri_TRI_S_ddd via l'attribut ID_TRI. L'attribut n'est pas renseigné (tri_1) ou alors la relation n'est pas vérifiée.",
            refError3.getMessage()
        );

        Assert.assertEquals(
            "L'objet test_b4 de la table TEST_B doit faire référence à un objet de la table N_prefixTri_TRI_S_ddd via l'attribut ID_TRI. L'attribut n'est pas renseigné (tri_1) ou alors la relation n'est pas vérifiée.",
