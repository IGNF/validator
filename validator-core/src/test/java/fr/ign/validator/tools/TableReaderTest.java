...
public class TableReaderTest {
    ...
    public void testReadZoneUrbaGML() {
        return;/*
                ...
                * ; assertTrue(file.exists()); try { TableReader reader =
                * TableReader.createTableReader(file,StandardCharsets.UTF_8);
                * 
                * String[] header = reader.getHeader() ;
                * 
                * assertTrue(Arrays.asList(header).contains("LIBELLE")) ; // patché, n'apparait
                * pas quand il est vide assertTrue(Arrays.asList(header).contains("NOMFIC")) ;
                * 
                * } catch (IOException e) { fail(e.getMessage()); } catch
                * (InvalidCharsetException e) { fail(e.getMessage()); }
                */
    }
