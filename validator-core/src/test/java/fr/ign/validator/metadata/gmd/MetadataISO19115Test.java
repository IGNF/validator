...
public class MetadataISO19115Test {
	...
	public void testRegress01() {
...

		{
			...
			assertEquals("-0.954544,48.150182,-0.847686,48.202573", extents.get(0).getBoundingBox().toString());
		}
		...
		assertEquals(SpatialRepresentationTypeCode.valueOf("vector"), metadata.getSpatialRepresentationType());
		...
		// onlineResources
		{
			...
			assertEquals("http://wxs-gpu.mongeoportail.ign.fr/externe/vkd1evhid6jdj5h4hkhyzjto/wms/v?",
					onlineResources.get(0).getUrl());
			assertEquals("OGC:WMS", onlineResources.get(0).getProtocol());
			assertEquals("Service de visualisation du Géoportail de l'Urbanisme", onlineResources.get(0).getName());
		}
	}
	...
	public void testRegress02() {
		...
		// constraints
		{
			...
			assertEquals(1, constraints.size());
			{
				...
				LegalConstraint constraint = (LegalConstraint) constraints.get(0);
				assertEquals(1, constraint.getUseLimitations().size());
				assertEquals("Licence Ouverte 1.0 http://www.data.gouv.fr/Licence-Ouverte-Open-Licence.",
						constraint.getUseLimitations().get(0));
				assertEquals(0, constraint.getUseConstraints().size());
				assertEquals(1, constraint.getAccessConstraints().size());
				assertEquals("otherRestrictions", constraint.getAccessConstraints().get(0));
				assertEquals(1, constraint.getOtherConstraints().size());
				assertEquals("Pas de restriction d'accès public selon INSPIRE",
...

		{
			List<Extent> extents = metadata.getExtents();
			assertEquals(1, extents.size());
			assertEquals("3.18713903,45.02106857,5.55596375,46.00650406", extents.get(0).getBoundingBox().toString());
		}
		...
		// getMetadataContact
		{
			ResponsibleParty contact = metadata.getMetadataContact();
			assertNotNull(contact);
			assertEquals("DDT 42 (Direction départementale des territoires de la Loire)",
			...
			assertEquals("https://lannuaire.service-public.fr/auvergne-rhone-alpes/loire/ddt-42218-01",
		...
		// getKeywords
		{
			...
			assertEquals("Aménagement Urbanisme/Assiette Servitude",
					StringUtils.join(keywords.get(0).getKeywords(), ';'));
			...
			assertEquals("Zones de gestion, de restriction ou de réglementation et unités de déclaration",
	...
	public void testRegress03() {
...

		{
			...
			assertEquals("4.01898628193261,49.2429346596619,5.42439068264001,50.150576343463",
		...
		assertEquals("Plan local d'urbanisme (PLU) - Balaives-et-Butz - approbation du 13/02/2014",
				metadata.getTitle());
		assertEquals(
		...
		// getKeywords
		{
			...
			assertEquals("PLU ; Plan d'occupation du sol ; Balaives-et-Butz ; urbanisme",
					StringUtils.join(keywords.get(0).getKeywords(), ';'));
			assertNull(keywords.get(0).getThesaurusName());
			assertNull(keywords.get(0).getThesaurusDate());
	...
	public void testRegress05() {
		...
		assertEquals("fr-120066022-orphan-residentifier-beed2702-07b7-4e4e-89fb-04b882e55334",
...

		{
			List<Extent> extents = metadata.getExtents();
			assertEquals(1, extents.size());
			assertEquals("2.048286199569702,42.691341400146484,0.4412877559661865,43.92110061645508",
		...
		// getMetadataContact
		{
			ResponsibleParty contact = metadata.getMetadataContact();
			assertNotNull(contact);
			assertEquals("DDT 31 (Direction Départementale des Territoires de Haute-Garonne)",
					contact.getOrganisationName());
	...
	public void testRegress06() {
...

		{
			...
			assertEquals("http://inspire.ec.europa.eu/metadata-codelist/SpatialDataServiceType/download http://inspire.ec.europa.eu/metadata-codelist/ApplicationProfile/view", locator.getApplicationProfileUrl());
		}
	}
