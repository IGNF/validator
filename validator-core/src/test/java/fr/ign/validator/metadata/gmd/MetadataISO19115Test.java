...
public class MetadataISO19115Test {
    ...
    protected Metadata getMetadataFromResource(String path) {
        File file = ResourceHelper.getResourceFile(getClass(), path);
    ...
    public void testIsMetadataFile() {
        File okFile = ResourceHelper.getResourceFile(getClass(), "/metadata/missing-charset.xml");
        File nokFile1 = ResourceHelper.getResourceFile(getClass(), "/metadata/not-a-metadatafile.xml");
        File nokFile2 = ResourceHelper.getResourceFile(getClass(), "/metadata/not-an-xml-file.xml");
    ...
    public void testRegress01() {
        ...
        assertEquals(LanguageCode.valueOf("fre"), metadata.getMetadataLanguage());
        ...
        assertEquals(
            "urn:isogeo:metadata:uuid:66484d70-3f8c-44cf-b0e5-98ac84426a2c",
            metadata.getFileIdentifier()
        );

        String[] expectedIdentifiers = {
            "66484d70-3f8c-44cf-b0e5-98ac84426a2c"
        };
        ...
        // constraints
        {
            ...
            assertEquals(1, constraints.size());
            {
                ...
                assertEquals(
                    "Pas de restriction d’accès public selon INSPIRE", constraint.getUseLimitations().get(0)
                );
                ...
                assertEquals(
                    "Informations non opposables au tiers.", constraint.getOtherConstraints().get(0)
                );
            }
        }

        {
            ...
            assertEquals(
                "-0.954544,48.150182,-0.847686,48.202573", extents.get(0).getBoundingBox().toString()
            );
        }
        ...
        assertEquals(LanguageCode.valueOf("fre"), metadata.getLanguage());
        ...
        assertEquals(
            SpatialRepresentationTypeCode.valueOf("vector"), metadata.getSpatialRepresentationType()
        );
        ...
        // onlineResources
        {
            ...
            assertEquals("OGC:WMS", onlineResources.get(0).getProtocol());
            assertEquals(
                "Service de visualisation du Géoportail de l'Urbanisme", onlineResources.get(0).getName()
            );
        }
    }
    ...
    public void testRegress02() {
        ...
        // constraints
        {
            ...
            assertEquals(1, constraints.size());
            {
                ...
                assertEquals(
                    ...
                    constraint.getUseLimitations().get(0)
                );
                assertEquals(0, constraint.getUseConstraints().size());
                assertEquals(1, constraint.getAccessConstraints().size());
                assertEquals("otherRestrictions", constraint.getAccessConstraints().get(0));
                ...
                assertEquals(
                    ...
                    constraint.getOtherConstraints().get(0)
                );
            }
        }

        {
            ...
            assertEquals(1, extents.size());
            assertEquals(
                "3.18713903,45.02106857,5.55596375,46.00650406", extents.get(0).getBoundingBox().toString()
            );
        }

        assertNull(metadata.getReferenceSystemIdentifier());

        assertEquals("Généalogie non renseignée", metadata.getLineage());

        assertEquals(
            "Servitudes - Lot A9 relatives aux zones agricoles protégées de la Loire",
            metadata.getTitle()
        );
        ...
        // getMetadataContact
        {
            ...
            assertNotNull(contact);
            assertEquals(
                "DDT 42 (Direction départementale des territoires de la Loire)",
            ...
            assertEquals(
                "https://lannuaire.service-public.fr/auvergne-rhone-alpes/loire/ddt-42218-01",
                contact.getOnlineResourceUrl()
            );
        ...
        // getKeywords
        {
            ...
            assertEquals(
                "Aménagement Urbanisme/Assiette Servitude", StringUtils.join(keywords.get(0).getKeywords(), ';')
            );
            ...
            assertEquals(
                "http://www.eionet.europa.eu/gemet/inspire_themes", keywords.get(2).getThesaurusUri()
            );
            assertEquals("2008-06-01", keywords.get(2).getThesaurusDate().toString());

            assertEquals("Parcelles cadastrales", StringUtils.join(keywords.get(3).getKeywords(), ';'));
            assertEquals("GEMET - INSPIRE Themes", keywords.get(3).getThesaurusName());
            assertEquals(
                "http://www.eionet.europa.eu/gemet/inspire_themes", keywords.get(3).getThesaurusUri()
            );
        ...
        // onlineResources
        {
            ...
            assertEquals("", onlineResources.get(0).getUrl());
            assertNull(onlineResources.get(0).getProtocol());
            assertEquals("Vue HTML des métadonnées sur internet", onlineResources.get(0).getName());
        }
    }
    ...
    public void testRegress03() {
        ...
        // constraints
        {
            ...
            }
            {
                ...
                assertEquals(1, constraint.getOtherConstraints().size());
                assertEquals(
                    "Pas de restriction d’accès publique", constraint.getOtherConstraints().get(0)
                );
            }
            {
...

        {
            ...
            assertEquals("177.0,-20.0,-178.0,20.0", extents.get(0).getBoundingBox().toString());
            assertEquals(
                "4.01898628193261,49.2429346596619,5.42439068264001,50.150576343463", extents.get(1).getBoundingBox()
                    .toString()
            );
        }
        ...
        assertEquals("Mise à jour du PLU de Balaives et Butz en 2014", metadata.getLineage());

        assertEquals(
            "Plan local d'urbanisme (PLU) - Balaives-et-Butz - approbation du 13/02/2014",
            metadata.getTitle()
        );
        assertEquals(
            "Plan local d'urbanisme (PLU) de la commune de Balaives-et-Butz (code INSEE 08042) approuvé le 13/02/2014",
            metadata.getAbstract()
        );
        assertEquals(ScopeCode.valueOf("dataset"), metadata.getType());
        ...
        // onlineResources
        {
            ...
            assertEquals("http://mairie-balaives-et-butz.fr/PLU", onlineResources.get(0).getUrl());
            assertEquals("WWW:LINK-1.0-http--link", onlineResources.get(0).getProtocol());
            assertEquals("site de la mairie de Belaives-Et-Butz", onlineResources.get(0).getName());
        }
    }
    ...
    public void testRegress04() {
...

        {
            List<Extent> extents = metadata.getExtents();
            assertEquals(4, extents.size());
            assertEquals("4.5,49.5,4.8,49.8", extents.get(0).getBoundingBox().toString());
        }
    ...
    public void testRegress05() {
...

        {
            ...
            assertEquals(1, extents.size());
            assertEquals(
                "2.048286199569702,42.691341400146484,0.4412877559661865,43.92110061645508", extents.get(0)
        ...
        // getMetadataContact
        {
            ...
            assertNotNull(contact);
            assertEquals(
                "DDT 31 (Direction Départementale des Territoires de Haute-Garonne)",
                contact.getOrganisationName()
            );
        ...
        // getKeywords (9 without thesaurusName, 1 with thesaurusName)
        {
            ...
            assertEquals(
                "Aménagement Urbanisme/Assiette Servitude", StringUtils.join(keywords.get(1).getKeywords(), ';')
            );
            ...
            assertEquals("données ouvertes", StringUtils.join(keywords.get(2).getKeywords(), ';'));
        ...
        // onlineResources
        {
            ...
            assertEquals("WWW:DOWNLOAD-1.0-http--download", onlineResources.get(0).getProtocol());
            assertEquals("Vue HTML des métadonnées sur internet", onlineResources.get(0).getName());
        }
    }

    /**
     * Test full gmd:CI_OnlineResource for locators
     */
    @Test
    public void testRegress06() {
        Metadata metadata = getMetadataFromResource("/metadata/06.xml");
        assertEquals("fr-000022358-plu20190101", metadata.getFileIdentifier());
        ...
        {
            OnlineResource locator = locators.get(0);
            assertEquals(
                "https://qlf-wxs-gpu.mongeoportail.ign.fr/externe/i9ytmrb6tgtq5yfek781ntqi/wms/v?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetCapabilities",
                locator.getUrl()
            );
            assertEquals("Lien vers le service WMS du Géoportail de l'urbanisme", locator.getName());
            assertEquals("OGC Web Map Service", locator.getProtocol());
            assertEquals("http://www.opengis.net/def/serviceType/ogc/wms", locator.getProtocolUrl());
            assertEquals("INSPIRE View Service", locator.getApplicationProfile());
            assertEquals(
                "http://inspire.ec.europa.eu/metadata-codelist/SpatialDataServiceType/download http://inspire.ec.europa.eu/metadata-codelist/ApplicationProfile/view",
                locator.getApplicationProfileUrl()
            );
            assertEquals(
                "Lien vers le service WMS du Géoportail de l'urbanisme (Description)",
