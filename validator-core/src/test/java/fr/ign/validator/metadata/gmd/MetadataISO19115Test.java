...
public class MetadataISO19115Test {
    ...
    public void testRegress02() {
        ...
        // constraints
        {
            ...
            assertEquals(1, constraints.size());
            {
                ...
                assertEquals(1, constraint.getUseLimitations().size());
                assertEquals(
                    "Licence Ouverte 1.0 http://www.data.gouv.fr/Licence-Ouverte-Open-Licence.", constraint
                        .getUseLimitations().get(0)
                );
                assertEquals(0, constraint.getUseConstraints().size());
                assertEquals(1, constraint.getAccessConstraints().size());
                assertEquals("otherRestrictions", constraint.getAccessConstraints().get(0));
                assertEquals(1, constraint.getOtherConstraints().size());
                assertEquals(
                    "Pas de restriction d'accès public selon INSPIRE", constraint.getOtherConstraints().get(0)
                );
            }
        }
        ...
        // getMetadataContact
        {
            ...
            assertNotNull(contact);
            assertEquals(
                "DDT 42 (Direction départementale des territoires de la Loire)", contact.getOrganisationName()
            );
            ...
            assertEquals(
                "https://lannuaire.service-public.fr/auvergne-rhone-alpes/loire/ddt-42218-01", contact
    ...
    public void testRegress03() {
        ...
        assertEquals(
            "Plan local d'urbanisme (PLU) - Balaives-et-Butz - approbation du 13/02/2014", metadata.getTitle()
        );
        assertEquals(
    ...
    public void testRegress05() {
        ...
        assertEquals(
            "fr-120066022-orphan-residentifier-beed2702-07b7-4e4e-89fb-04b882e55334", metadata.getIdentifier()
        );
        ...
        // getMetadataContact
        {
            ...
            assertNotNull(contact);
            assertEquals(
                "DDT 31 (Direction Départementale des Territoires de Haute-Garonne)", contact.getOrganisationName()
            );
