 ...
 * 
 * TODO complete {@link LegacyFormatTest} and escape strings (convert null to ""
 * and escape '|' in message)
 * 
...
public class LegacyFormat {
    ...
    /**
     * Header | [code] | [fileModel] | [level] | [column] | [value] | [modelValue] |
     * [message]
     * 
     * @param error
     * @return
    ...
    private String writeHeader(ValidatorError error) {
        ...
        result += " | " + error.getLevel(); // error type
        result += " | " + error.getAttribute(); // attribute (deprecated, can be found in messages) => Champ (déprécié,
                                                // présent dans les messages)
        result += " | "; // current (deprecated, can be found in messages) =>Courant (déprécié, présent
                         // dans les messages)
    ...
    /**
     * Feature | [code] | [fileModel] | [level] | [column] | [line] | [currentvalue]
     * | [type] | [model] | [message]
     * 
     * WARNING : [currentvalue] and [model] (size, allowed values, etc.) are
    ...
    private String writeFeature(ValidatorError error) {
        ...
        result += " | "; // value (deprecated, can be found in messages) => valeur (déprécié, présent
                         // dans les messages)
        result += " | "; // type (deprecated, can be found in messages) => taille/type (déprécié, présent
                         // dans les messages)
        result += " | " + error.getDocumentModel(); // model
        result += " | " + error.getMessage(); // message
        return result;
