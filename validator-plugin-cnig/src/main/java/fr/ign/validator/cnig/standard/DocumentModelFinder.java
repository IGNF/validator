...
/**
 * 
 * Find DocumentModel for CNIG documents according to document name and metadata
 * specification
 * 
 * @author MBorne
 *
...
public class DocumentModelFinder {
     ...
     * 
     * Note that candidates are sorted by descendant name so that candidate[0] is
     * the last version
     * 
     * @param documentName
     * @return
    ...
    public DocumentModel findByDocumentPath(File documentPath) throws IOException {
        ...
        for (DocumentModel candidate : candidates) {
            if (candidate.getName().endsWith(version)) {
                log.info(
                    MARKER, "findByDocumentPath({}) : candidate {} match version {}", documentPath, candidate.getName(),
                    version
                );
                return candidate;
            }
    ...
    private Specification findMetadataSpecification(File documentPath) {
        ...
        Collection<File> candidateFiles = FileUtils.listFiles(documentPath, new String[] {
            "xml", "XML"
        }, true);
